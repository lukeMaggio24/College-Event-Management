...
router.post("/", (req, res) => {
    ...
    const query = "UPDATE rso SET email = CONCAT(IFNULL(member_emails, ''), ?, ' ') WHERE rso_name = ?";
    ...
    db.query(query, [email, rso_name], (error, results) => {
        if(err) {
            ...
            const queryIncrement = "UPDATE rso SET numOfMembers = numberOfMembers + 1 Where rso_name = ?";
